...
exports.downloadFolderacls = function() {
  ...
  axios({
    ...
    .then(result => {
      if (result.data) {
        const req = result.data.filter((entry) => {
          return entry.link.map((link) =>{
            if (link.___rel==='self') {
              if (link.___href.charAt(1)!=='_') {
                return true
              }else {
                return false
              }
        ...
        fsasync.writeFileSync( './setup/_settings/folderacls.json' , JSON.stringify(req ,null , '\t'))
        console.log('folderacls.json is saved.')
      }
    })
    .catch(error => {
      console.log('Get failed.:'+error)
...
exports.downloadData = function(folder) {
  ...
  console.log('fetching /d/'+folder)

  const getfeed = function(url,i,r) {
    axios({
      url: url,
      method: 'get',
      responseType: 'stream',
...
//        'Accept-Encoding': 'deflate'
      ...
      .then(result => {
        if (result.data) {
          const nextpage = result.headers['x-vtecx-nextpage']
          result.data.pipe(fsasync.createWriteStream( './data/'+folder+i+'.dat')).on('finish', function() {
            console.log('./data/'+folder+i+' is saved.')
            if (nextpage!=undefined) {
              getfeed(url+'&p='+nextpage,++i,0)
            }else {
              console.log('done.')
            }
          })
  ...
  getfeed(path+'/d/'+folder+'?f&_nometa&l=50000&x',0,0)
...
const sendfile = function(file, param, isdirectory, isd, iss) {
