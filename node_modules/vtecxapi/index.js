...
const sortentry = function(feed,url) {

  const s = url.indexOf('s=')
  let s2
  let p
  if (s === -1) {
    return feed
  }else {
    s2 = url.substring(s+2)
    p = s2.indexOf('&')
    let item
    if (p!=-1) {
      s2 = s2.substring(0,p)
    ...
    if (s3[1]===undefined || s3[1]==='asc') {
      if (s4[1]===undefined) {
        feed.sort(function(a, b) {
          if (a[s4[0]]===undefined) return 1
          if (b[s4[0]]===undefined) return -1
          if (a[s4[0]] >= b[s4[0]]) {
            return 1
          } else {
            return -1
...
let result = []
function getFeed(url,targetServiceName,targetServiceKey) {
  let r
  if (targetServiceName&&targetServiceKey) {
    r = ReflexContext.getFeed(url,targetServiceName,targetServiceKey)
  }else {
    r = ReflexContext.getFeed(url)
  }
  if (!r) {
    return result
  }
  if (r.feed.link===undefined) {
    result = result.concat(r.feed.entry)
    return
  }else {
    result = result.concat(r.feed.entry)
    let p
    if (url.indexOf('?') != -1) {
      p = '&p=' + r.feed.link[0].___href
    } else {
      p = '?p=' + r.feed.link[0].___href
    }
    getFeed(url+p,targetServiceName,targetServiceKey)
  }
}
  ...
  try {
      if (force) {
        ...
        result = []
        if (targetServiceName&&targetServiceKey) {
          getFeed(url,targetServiceName,targetServiceKey)
        }else {
          getFeed(url)
        }
        if (result.length>0) {
          return sortentry(result,url)
        }else
        return []
      }else {
        var result2 = ReflexContext.getFeed(url)
        if (result2&&result2.feed&&result2.feed.entry) {
           var result3 = sortentry(result2.feed.entry,url)
          if (result2.feed.link!==undefined) {
//            console.log('reflexcontext.getFeed(): next page exists.')
            result3[0].rights = result2.feed.link[0].___href
          }
          return result3
        }else
        return []
 ...
 * @param  {feed} feed - feed to delete users
 * @returns  {feed} - results
 */
vtecxapi.deleteUser = function(feed) {

  try {
    return ReflexContext.deleteUser(feed);
  } catch (e) {
   if (e.message === 'ReflexContext is not defined') {
      console.log('ReflexContext.deleteUser() is called but not executed.');
      return '';
    } else {
      console.log('reflexcontext.deleteUser : '+e.message);
      throw e;
    }
  }
}
...
/**
 * @description Get string from session by name.
 * @param  {string} name - sessionkey
 * @returns  {string} - results
 */
vtecxapi.getSessionString = function(name) {

  try {
    return ReflexContext.getSessionString(name);
  } catch (e) {
   if (e.message === 'ReflexContext is not defined') {
      console.log('ReflexContext.getSessionString() is called but not executed.');
      return '';
    } else {
      console.log('reflexcontext.getSessionString : '+e.message);
      throw e;
    }
  }
}
...
/**
 * @description Delete string of session.
 * @param  {string} name - sessionkey
 */
vtecxapi.deleteSessionString = function(name) {

  try {
    ReflexContext.deleteSessionString(name);
  } catch (e) {
   if (e.message === 'ReflexContext is not defined') {
      console.log('ReflexContext.deleteSessionString() is called but not executed.');
      return '';
    } else {
      console.log('reflexcontext.deleteSessionString : '+e.message);
      throw e;
    }
  }
}
 ...
 * @param  {props} tablenames - key:entity's prop name,value:BigQuery table name
 */
vtecxapi.postBQ = function (request,async,tablenames) {

  try {
      if (tablenames===undefined) {
        ReflexContext.postBQ(request,async)
      }else {
        ReflexContext.postBQ(request,tablenames,async)
      }
  }catch (e) {
    if (e.message === 'ReflexContext is not defined') {
 ...
 * @param  {props} tablenames - key:entity's prop name,value:BigQuery table name
 */
vtecxapi.deleteBQ = function (keys,async,tablenames) {
